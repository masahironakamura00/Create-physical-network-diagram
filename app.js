...
class NetworkApp {
    ...
    drawNode(node) {
        ...
        let touchMode = null; // 'move' or 'connect'
        let touchCurrentOffsetX = 0;
        let touchCurrentOffsetY = 0;
        let touchStartDistance = 0;
        let touchAreaType = null; // 'center' or 'frame'
        ...
        const handleTouchStart = (e) => {
            ...
            if (distFromCenter < 40) {
                // ノード中央：タップで削除またはロングプレスで移動
                touchAreaType = 'center';
            ...
            } else if (distFromCenter >= 40 && distFromCenter <= 55) {
                // ノード周囲の枠：ロングプレスで接続作成
                touchAreaType = 'frame';
                app.touchLongPressTimer = setTimeout(() => {
                    isTouchDragging = true;
        ...
        const handleTouchMove = (e) => {
            ...
            if (isTouchDragging) {
                ...
                if (touchMode === 'connect') {
                    ...
                    // ノード移動モード
                    group.setAttribute('transform', `translate(${touchCurrentOffsetX}, ${touchCurrentOffsetY})`);
                }
        ...
        const handleTouchEnd = (e) => {
            const wasLongPressed = isTouchDragging;
            ...
            if (isTouchDragging) {
                ...
                if (touchMode === 'move' && hasMoved) {
                    // ノード移動モード：位置を確定
                    node.x = nodeStartX + touchCurrentOffsetX;
                    node.y = nodeStartY + touchCurrentOffsetY;
                    node.hasCustomPosition = true;
                    app.redrawDiagram();
                } else if (touchMode === 'connect' && hasMoved) {
                ...
                touchMode = null;
            } else if (!wasLongPressed && !hasMoved && touchAreaType === 'center') {
                // ロングプレスなしのタップ（短いタップ）でノード中央をタップした場合は削除
                app.removeNodeFromDiagram(node.id);
            }
